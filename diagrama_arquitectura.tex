...
\begin{tikzpicture}[
  ...
  resp/.style={text=red, font=\footnotesize\bfseries, inner sep=1pt},
]
...
\node[filtro] (verif) at (19.0,4.5) {Verificación};
\node[filtro] (norm)  at (22.2,4.5) {Normalización};

% ---------- Fuente y sumidero (separados para que la conexión punteada respire) ----------
\node[exterior] (portales) at (-1.1,4.5) {Portales\\digitales};
\node[exterior] (presenta) at (26.3,4.5) {Capa de\\presentación\\\scriptsize(tableros, reportes)};

% ---------- Puertos (cuadraditos en bordes O/E, y=4.5) ----------
\foreach \f in {extr,enriq,comp,invoc,deco,verif,norm}{
...
\node[pnum] at ($(verif-in) +(-0.18,0.32)$) {11};
\node[pnum] at ($(verif-out)+(0.18,0.32)$)  {12};
\node[pnum] at ($(norm-in)  +(-0.18,0.32)$) {13};
\node[pnum] at ($(norm-out) +(0.18,0.32)$)  {14};
...
\draw[tubo] (deco-out)  -- (verif-in);
\draw[tubo] (verif-out) -- (norm-in);
...
% viven en el sintonizador de Composición. Verificación expone sintonizador:
% el criterio editorial de validez y la selección del ejecutor de la verificación.
...
\node[sintbox] (sint-verif) at (19,7.3)   {Criterio de validez\\y ejecutor};
\node[sintbox] (sint-norm)  at (22.2,7.3) {Criterios de\\consolidación};
\draw[sint] (sint-extr.south)  -- (extr.north);
\draw[sint] (sint-comp.south)  -- (comp.north);
\draw[sint] (sint-verif.south) -- (verif.north);
...
\node[repos, minimum width=21cm, minimum height=1.2cm] (repo) at (12.6,0.3)
  {Repositorio común --- PostgreSQL / ORM de Odoo};
\foreach \f in {extr,enriq,comp,invoc,deco,verif,norm}{
  \draw[memoria] (\f.south) -- (\f.south |- repo.north);
}

% ---------- Marcadores de responsabilidades transversales [A]–[F] (Sección 1.3 de main.tex) ----------
% [A] experto · [B] estudio · [C] generativa · [D] control editorial · [E] procedencia · [F] preexistente.
% Se ubican bajo cada filtro que realiza la responsabilidad, junto a la fuente y sobre el repositorio.
\node[resp, anchor=west] at ($(extr.south)  +(0.12,-0.74)$) {[A]\,[F]};
\node[resp, anchor=west] at ($(enriq.south) +(0.12,-0.74)$) {[C]};
\node[resp, anchor=west] at ($(comp.south)  +(0.12,-0.74)$) {[A]\,[B]};
\node[resp, anchor=west] at ($(invoc.south) +(0.12,-0.74)$) {[C]};
\node[resp, anchor=west] at ($(deco.south)  +(0.12,-0.74)$) {[B]\,[C]};
\node[resp, anchor=west] at ($(verif.south) +(0.12,-0.74)$) {[A]\,[B]\,[D]};
\node[resp, anchor=west] at ($(norm.south)  +(0.12,-0.74)$) {[A]\,[B]\,[D]};
\node[resp, anchor=north] at ($(portales.south)+(0,-0.1)$) {[F]};
\node[resp, anchor=east]  at ($(repo.east)+(-0.25,0)$) {[E]};
...
\node[anchor=north west, font=\footnotesize] (leyenda) at (-1.5,-1.6) {%
  \begin{tabular}{@{}cl@{\hspace{10pt}}cl@{}}
    ...
    & \tikz[baseline=-0.6ex]{\draw[thick,dash pattern=on 3pt off 2pt,rounded corners=2pt] (0,-0.13) rectangle (0.5,0.13);}
      & filtro compuesto \\[5pt]
    \multicolumn{4}{@{}l}{\textcolor{red}{\bfseries [A]}\enspace responsabilidad transversal (\textsection~1.3 del documento):} \\[2pt]
    \multicolumn{4}{@{}l}{\quad [A] experto \(\cdot\) [B] estudio \(\cdot\) [C] generativa \(\cdot\) [D] control editorial \(\cdot\) [E] procedencia \(\cdot\) [F] preexistente} \\
  \end{tabular}
};

% ================= TABLA DE PUERTOS =================
\node[anchor=north west, font=\footnotesize] (tabla) at (11.4,-1.6) {%
  \begin{tabular}{@{}rll@{\hspace{14pt}}rll@{}}
    ...
    1  & Extracción (ent.)      & flujo del portal       & 8  & Invocación (sal.)     & \texttt{RespuestaIAG[M]} \\
    2  & Extracción (sal.)      & \texttt{Noticia[N]}    & 9  & Decodificación (ent.) & \texttt{RespuestaIAG[M]} \\
    3  & Enriquecimiento (ent.) & \texttt{Noticia[N]}    & 10 & Decodificación (sal.) & \texttt{Análisis[M]} \\
    4  & Enriquecimiento (sal.) & \texttt{Embedding[N]}  & 11 & Verificación (ent.)   & \texttt{Análisis[M]} \\
    5  & Composición (ent.)     & \texttt{Embedding[N]}  & 12 & Verificación (sal.)   & \texttt{Veredicto[M]} \\
    6  & Composición (sal.)     & \texttt{Prompt[M]}     & 13 & Normalización (ent.)  & \texttt{Veredicto[M]} \\
    7  & Invocación (ent.)      & \texttt{Prompt[M]}     & 14 & Normalización (sal.)  & \texttt{Resultado} \\
  \end{tabular}
};
