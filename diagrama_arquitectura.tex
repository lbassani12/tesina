...
\node[pnum] at ($(enriq-out)+(0.18,0.32)$)  {4};
\node[pnum] at ($(comp-in)  +(-0.18,0.32)$) {5};
...
\node[anchor=north west, font=\footnotesize] (tabla) at (10.6,-1.6) {%
  \begin{tabular}{@{}rll@{\hspace{14pt}}rll@{}}
    \multicolumn{6}{@{}l}{\textbf{Referencia de puertos}\quad\footnotesize(\texttt{X[]} denota un recordset de \texttt{X}; el contenido reside en el repositorio común)} \\[3pt]
    1  & Extracción (ent.)      & flujo del portal     & 7  & Invocación (ent.)     & \texttt{Prompt[]} \\
    2  & Extracción (sal.)      & \texttt{Noticia[]}   & 8  & Invocación (sal.)     & \texttt{RespuestaIAG[]} \\
    3  & Enriquecimiento (ent.) & \texttt{Noticia[]}   & 9  & Decodificación (ent.) & \texttt{RespuestaIAG[]} \\
    4  & Enriquecimiento (sal.) & \texttt{Noticia[]}   & 10 & Decodificación (sal.) & \texttt{Análisis[]} \\
    5  & Composición (ent.)     & \texttt{Noticia[]}   & 11 & Normalización (ent.)  & \texttt{Análisis[]} \\
    6  & Composición (sal.)     & \texttt{Prompt[]}    & 12 & Normalización (sal.)  & \texttt{Resultado} \\
  \end{tabular}
};
